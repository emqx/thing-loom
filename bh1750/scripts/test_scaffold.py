#!/usr/bin/env python3
"""Small offline check for Zero-backed BH1750 project generation."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        firmware = (output / "bh1750-demo.ino").read_text()
        ...
        assert "constexpr uint8_t SDA_PIN = 6;" in firmware
        assert "constexpr uint8_t SCL_PIN = 7;" in firmware
        assert 'constexpr char MQTT_TOPIC[] = "bh1750/readings";' in firmware
        ...
        assert '"topic":"bh1750/readings"' in web_config
    ...
    with tempfile.TemporaryDirectory() as temp:
        output = Path(temp) / "bh1750-custom"
        ...
        env = (output / ".env").read_text()
        firmware = (output / "bh1750-custom.ino").read_text()
        web_config = (output / "web/public/mqtt-config.js").read_text()
