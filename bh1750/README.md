...
This skill builds a complete ESP32-C6 and BH1750FVI ambient-light monitor: wiring, project generation, a disposable Zero EMQX namespace, firmware flashing, serial verification, and a local or Cloudflare-hosted live dashboard.
...
The agent guides the wiring, collects secrets through hidden local prompts, creates an isolated Zero EMQX namespace after approval, flashes the firmware, verifies a real lux message with MQTTX, and opens the dashboard.
...
Zero EMQX provides authenticated MQTTS and WSS. The scaffold writes Wi-Fi, MQTT, and optional Cloudflare credentials to `data/<project>/.env` with mode `600`. Generated projects and all `.env` files are ignored by Git.
